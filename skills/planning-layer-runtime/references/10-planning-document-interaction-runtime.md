...
- 09：现有系统哪些能力可以保留，哪些旧流程或旧逻辑绝不能再参与，这次变化要从哪里切换到新流程，出现问题时应该回退到什么可接受状态；涉及数据持久化时，还要基于第一阶段业务约束和项目证据形成数据库候选方案，覆盖现有库复用或新建、数据库类型、各环境位置、远程库与现有资料状态、迁移、回退、备份、保留删除和敏感数据边界。物理方案允许在第二阶段确认，不得为了纯技术字段把第一阶段变成配置访谈。
...
- 进入第二阶段和每次恢复文档确认时，必须把用户当前明确要求、本期 Discovery 中反复体现的用词与纠正方式、无冲突的高置信长期画像合并为 `current-interaction.yaml.explanation_adaptation`；当前表达优先，无法判断时使用 `plain_language`，不得只靠压缩后的聊天摘要猜测。
- 正式文档可以保留专业术语；用户态解释是否需要逐词说明由 `explanation_adaptation` 决定。`plain_language` 首次出现术语时必须补一句业务解释和实际影响，`adaptive` 解释首次出现或可能歧义的词，`technical_concise` 可直接展示字段和值但仍说明业务与交付影响。
- 第一阶段使用业务白话的硬规则不得因为第二阶段允许术语而失效，也不得依据技术型画像回头把 Discovery 变成数据库、接口或架构配置问卷。
- 不得在用户态展示画像分类、置信度或“你不懂技术”等判断；用户要求调整解释方式时立即更新本期适配快照，长期偏好只在最终 Interaction Preference Consolidation 中按证据更新。
...
### 5.2 06/09 Database Explanation And Confirmation Gate

当前文档为 06 且本期涉及持久化时，人话总结必须说明：

- 哪些业务记录必须在关闭页面或服务重启后仍然保留。
- 哪些变化必须由系统判断，哪些只是页面临时状态。
- 旧记录是否继续影响新流程，以及测试、预发布和正式数据为什么必须隔离。
- 06 只确认业务数据与变化规则；具体使用什么数据库、放在哪里和如何迁移会在 09 确认。

当前文档为 09 且本期涉及数据库或持久化时，必须在聊天回复中直接展示数据库方案摘要，不得只提供 Markdown 路径。摘要至少覆盖：

- 当前有没有已核实、可以复用的数据库与现有数据。
- 复用、扩展、新建或替换的草案方向。
- 目标数据库和版本，以及它为什么适合已确认的使用方式。
- 本地开发、测试、预发布、正式环境分别在哪里运行，是否互相隔离。
- 是否需要远程数据库，当前是已提供、后续可提供、不需要、无法提供还是未知。
- 用户后续能否提供 Schema/Migration、脱敏样例、备份或受控只读入口；不得索取账号密码。
- 是否迁移旧数据、失败后如何回退、备份恢复、保留删除和敏感数据边界。
- 当前结论来自项目证据、用户确认、解释后的推荐默认值还是明确委托；哪些缺失会阻断开发、联调或上线。

交互规则：

- `plain_language` 先说实际使用效果，再在括号中给出必要术语并用一句话解释。例如“在本机启动一套独立的数据环境（本地 PostgreSQL，也就是保存多人业务记录的软件）”。
- `adaptive` 保留必要术语，但只解释第一次出现或可能改变用户判断的词。
- `technical_concise` 可以直接展示引擎、版本、拓扑、Migration、Seed、只读访问和阻断状态，但不得省略业务影响和未就绪事实。
- 用户不懂时可以确认解释后的推荐默认值；确认后才写 `recommended_default_confirmed`。用户懂技术或提出修改时，其当前明确反馈优先于长期画像和原候选方案。
- 用户反馈只是要求解释时保持 09 为草案；反馈修改数据库方向时先回写 09，再精确重建受影响下游。
- `blocking_open` 必须说明缺什么、由谁提供、最晚何时提供以及阻断范围，并移交 12；不得用“后续再看”引导用户确认 09。
- 专业文档完整性不因人话解释而降低；解释层也不得增加文档中不存在的数据库事实。
