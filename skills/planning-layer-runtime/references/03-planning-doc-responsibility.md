...
03 不得默认用户已登录、已有会话、已有组织/租户上下文、已有项目/区域范围或天然有操作资格。若这些前置由既有系统提供，也必须写清楚“由既有体验承接”，以及恢复成功后进入哪个 PAGE。
...
- 本期允许复用的技术基础。
- 本期禁止复用的旧业务语义。
- 当前数据库与持久化基线是已核实、部分核实、无现有实现还是未知。
- 现有数据库、结构资料、脱敏数据、备份或受控只读入口是否可复用或可由后续提供。
...
- “独立数据域”只表达业务语义、来源、状态、权限和写入边界，不得作为创建期次物理模块或数据命名空间的依据。
...
- 13、14、15 只在 `requires_execution_handoff: true` 时适用。
- 13 不适用时，14、15 必须同时不生成。
- 不生成 13 不等于 Planning 无法完成；`planning_only` Handoff 可以合法结束 Planning。
- 需要开发或存在其他后续工程执行任务时，禁止通过动态装配跳过 13。
- 需要生成 13 时，必须同时装配并确认 11、12；不需要生成 13 的轻量规划，仅在确有风险、依赖或待决事项时装配 12。
- 本节只声明 12–15 的适用边界；条件式结束链路由 `07-planning-conversation-runtime.md` 和 `10-planning-document-interaction-runtime.md` 维护。
...
- 使用 TASK / FLOW / DOMAIN / API / PERM 等 Planning ID 或期次编号暗示代码、数据库、API、权限或迁移命名。
- 09 未批准 `create_stable_business_domain` 时暗示必须创建新物理模块。
...
- 03：用户操作、失败、恢复和取消路径。
- 05：页面输入、提示和交互边界。
- 06：状态、业务事实、唯一性、修改删除和数量规则。
- 07：请求字段约束、允许/禁止值、错误和恢复语义。
- 08：权限和数据范围。
- 09：实现承接策略、数据库与持久化决策、环境拓扑与架构边界。
- 10：外部能力官方限制、失败和降级要求。
- 11：如何证明参数边界正确。
- 12：缺失参数形成的 RISK、DEP 或 OPEN。
- 13：汇总 Task Ready 所需参数是否闭合，不重复定义上游事实。

下游发现参数缺失时，必须回到真正拥有该事实的上游文档修正。
...
三个 13 Gate、用户确认、14/15 派生与 Handoff 准备的唯一顺序以 `04-planning-format-spec.md#13-to-14--15-framework-derivation-rule` 为准；本节不复制该顺序。该规则仅适用于 `requires_execution_handoff: true`。
...
最终人话总结不是新的 00–15 SoT，也不再次确认 14 或 15；它只汇总已确认事实，作为本期规划正式结束前的最后一次整体确认。用户指出偏差时，必须回写真正拥有该事实的上游 SoT，不得只改总结。
