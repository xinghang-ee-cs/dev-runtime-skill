...
- 一期关闭后，该期 00–15 进入历史只读；只允许受控修正事实错误，以及按 `14-bugfix-case-governance.md` 在其 `bugfixes/` 下追加不参与 baseline hash 的 Bugfix Case。独立新增需求默认从最新 `PROJECT-CURRENT-BASELINE.md` 启动下一期。
...
- 上游文档未确认时，允许按同一批次依赖拓扑生成状态为 `草案` 的下游正文以完成全链路检查；下游不得标记确认、冻结或进入 Handoff，且上游修正命中时必须重建受影响草案。
- 每份文档只负责自己的职责，不允许跨文档写内容。
- 下游发现上游错误时，必须回到上游修正。
- 本期适用的 00–12 与可选 13 必须先按依赖顺序一次性批量生成草案，再按 confirmation queue 单独解释、单独确认；批量生成中途不得向用户逐份确认。14、15 仍只在 13 已确认后自动派生，不进入初始批量草案，也不需要独立确认。
- 文档结论必须可追踪到已确认信息。
- 未确认内容不得写成事实。
- 所有关键实体必须具备唯一 ID。
- 所有关键结论必须可追踪到来源。
