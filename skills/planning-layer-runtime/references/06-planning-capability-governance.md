...
- 1–6. 目标、定义、分类、发现、ID 与 Registry
- 7–9. 官方事实、开发入口证据与验收要求
- 10–14. 实现、复核、代码证据与门禁
- 15–18. 文档装配、任务拆分、验收范围与职责边界
...
这里禁止的是由 10 擅自设计实现命名和加载方式，不阻止第二阶段在确认 12 时，为了让用户完成真实前置条件而展示项目中**已经存在并已核实**的公开示例键名、安全配置入口、官方控制台路径或秘密管理渠道。此类信息只作为 12 配置类 DEP 的执行指引与脱敏就绪证据，不回写成 10 的实现合同；只允许核对键或入口是否存在，严禁读取、展示或持久化秘密值。
...
| 数据持久化、现有/远程数据库复用、新建数据库、数据库环境或数据迁移 | `06-数据模型与状态流转.md` + `09-架构设计与关键决策.md`；同时加载 `13-planning-database-persistence-contract.md` |
...
- S/M 级变更只加载必要文档。
- 未触发的文档不强制生成。
- 一旦本期会新增或修改持久化数据、数据库结构、数据库环境、远程数据库依赖或数据迁移，06 与 09 都是必需职责；不得只生成 06 后让执行层自行选择数据库，也不得只生成 09 后跳过业务事实与状态来源。
- 文档装配结论必须写入 `涉及文档`。
